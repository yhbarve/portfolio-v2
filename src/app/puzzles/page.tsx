...
export default async function PuzzlesPage({ searchParams }: PuzzlesPageProps) {
  ...
  const puzzles = allPuzzles
    .filter((p: Puzzle) => p.published)
    .filter((p: Puzzle) => {
      if (!categoryFilter) return true;
      return p.difficulty === categoryFilter;
    })
    .sort((a: Puzzle, b: Puzzle) => +new Date(b.date) - +new Date(a.date));
  ...
  return (
    <div className="lg:pb-24">
      <div className="flex flex-wrap gap-2 pb-6 pt-2">
        ...
        {PUZZLE_CATEGORIES.map((cat: PuzzleCategory) => (
          <Link
            key={cat}
      ...
      <div className="flex flex-col mb-12">
        {puzzles.length > 0 ? puzzles.map((p: Puzzle) => {
          return (
            <div
              key={p.slug}
